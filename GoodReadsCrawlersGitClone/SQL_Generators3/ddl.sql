...
-- Index
CREATE INDEX Book_Place_Index ON BookPlace(book_id, place_id);
...
SELECT
  title AS "Title",
  publishedDate as "Publication Date",
  description AS "Description",
  isbn_10 AS "ISBN10",
  isbn_13 AS "ISBN13",
  pageCount AS "Number of Pages",
  averageRating AS "Average Rating",
  ratingsCount AS "Number of Ratings",
  (SELECT amount || ', ' || currencyCode FROM RetailPrice WHERE book_id = b.book_id) AS "Retail Price",
  (SELECT amount || ', ' || currencyCode FROM ListPrice WHERE book_id = b.book_id) AS "List Price"
FROM Book b;
...
SELECT
  ...
  (SELECT COUNT(*) FROM Author) AS "Total Number of Authors",
  (SELECT COUNT(*) FROM Reviewer) AS "Total Number of Reviewers"
FROM Person p;
