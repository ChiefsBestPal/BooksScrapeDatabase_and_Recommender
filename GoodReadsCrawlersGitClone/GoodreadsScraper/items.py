...
#     def add_reviewers_and_reviews(self, response):
#         json_data = json.loads(response.text)
#         reviewers = {}
#         reviews = []

#         # Extract reviewer information
#         review_creators = json_data.get('props', {}).get('pageProps', {}).get('apolloState', {}).get('Review', {})
#         for review_id, review_data in review_creators.items():
#             creator_ref = review_data.get('creator', {}).get('__ref')
#             if creator_ref and creator_ref.startswith('User:'):
#                 reviewer_id = creator_ref.split(':')[1]
#                 reviewers[reviewer_id] = {
#                     'name': review_data.get('creator', {}).get('name'),
#                     'id': reviewer_id,
#                     'followerCount': review_data.get('creator', {}).get('followerCount'),
#                     'isAuthor': review_data.get('creator', {}).get('isAuthor')
#                 }

#         # Extract review information
#         review_objects = json_data.get('props', {}).get('pageProps', {}).get('apolloState', {}).get('User', {})
#         for review_id, review_data in review_objects.items():
#             if review_id.startswith('Review:'):
#                 review = {
#                     'updatedAt': safe_parse_date(review_data.get('updatedAt')),
#                     'createdAt': safe_parse_date(review_data.get('createdAt')),
#                     'text': review_data.get('text'),
#                     'rating': review_data.get('rating'),
#                     'likeCount': review_data.get('likeCount')
#                 }
#                 creator_ref = review_data.get('creator', {}).get('__ref')
#                 if creator_ref and creator_ref.startswith('User:'):
#                     reviewer_id = creator_ref.split(':')[1]
...
#         self.add_value('reviewers', reviewers.values())
#         self.add_value('reviews', reviews)